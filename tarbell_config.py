...
def apply_glossary_tags(context, text_blob):
    """
    Testing, for the moment, whether we can programattially add glossary tags to things. The filter
    will recieve chunks of a story (blobs of text several grafs long) and look for all instances of 
    predefined terms and wrap them in span tags with an identifiying data-ID.
    """
    ...
    for term in term_lookup:
        ...
        if search_term.upper() in retval.upper():
            retval = find_term(retval, term, search_term)
            # # If the desired term/substr is found in the blob of story text, then process.
            # search_term_id = term["ID"] #data-* attribute of related term
...
def find_term(text_blob, term, search_term):
    # If the desired term/substr is found in the blob of story text, then process.
    search_term_id = term["ID"] #data-* attribute of related term
    search_term_length = len(search_term) 
    search_term_starting_index = retval.upper().find(search_term.upper())
    retval = insert_glossary_tags(text_blob, search_term_starting_index, search_term_length, search_term_id)
...
S3_BUCKETS = {
    ...
    "production": "apps.chicagotribune.com/news/watchdog/property-taxes-cook-county",
